...
/////// Diffuse:
                ...
                float4 node_2991 = _Time + _TimeEditor;
                float2 node_1695 = (i.uv0+node_2991.g*float2(0,0.05));
                float4 _cloud2_var = tex2D(_cloud2,TRANSFORM_TEX(node_1695, _cloud2));
                float2 node_2887 = (i.uv0+node_2991.g*float2(-0.05,0));
...
/////// Diffuse:
                float NdotL = max(0.0,dot( normalDirection, lightDirection ));
                float3 directDiffuse = max( 0.0, NdotL) * attenColor;
                float4 node_3038 = _Time + _TimeEditor;
                float2 node_1695 = (i.uv0+node_3038.g*float2(0,0.05));
                float4 _cloud2_var = tex2D(_cloud2,TRANSFORM_TEX(node_1695, _cloud2));
                float2 node_2887 = (i.uv0+node_3038.g*float2(-0.05,0));
                float4 _cloud3_var = tex2D(_cloud3,TRANSFORM_TEX(node_2887, _cloud3));
                float4 _valueblend_var = tex2D(_valueblend,TRANSFORM_TEX(i.uv0, _valueblend));
                float3 diffuseColor = saturate(((saturate((saturate(( _cloud2_var.rgb > 0.5 ? (1.0-(1.0-2.0*(_cloud2_var.rgb-0.5))*(1.0-_cloud1_var.rgb)) : (2.0*_cloud2_var.rgb*_cloud1_var.rgb) ))*_cloud3_var.rgb))*_maincolor1.rgb)*(_valueblend_var.rgb+_maincolor1.rgb)));
